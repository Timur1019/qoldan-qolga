...
export default function useAdminAdSidebarBanners() {
  ...
  const save = (e) => {
    ...
    if (!body.imageUrl || !body.linkUrl) {
      setError('Нужны картинка и ссылка')
      return
    }
    const req = editingId
      ? adminApi.updateAdSidebarBanner(editingId, body)
      : adminApi.createAdSidebarBanner(body)
    req
      .then(() => {
